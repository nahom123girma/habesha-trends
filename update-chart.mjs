...
async function main() {
  ...
  const songs = ytIds.map(function (ytId) {
    ...
    return {
      // --- Site-required identity (preserved from previous data.json) ---
      // 'id' powers routing, click handlers, detail pages, and country
      // filtering ('e' prefix = Eritrean). MUST exist in previous data.json
      // for the song to render correctly on the site.
      ...
      score:       (prev && prev.score) != null ? prev.score : 0,
  ...
  }).filter(function (x) { return x && x.id; });    // drop entries with no id (would break site routing)
